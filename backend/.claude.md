...
- **Draft operations**: SaveDraft handles both INSERT (create) and UPDATE (edit). Check if `draft.ID > 0` to decide operation. UpdateDraft service method validates org ownership then calls SaveDraft.
