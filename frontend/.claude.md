...
### API Communication with SWR

#### Data Fetching Pattern with SWR
- **Always use SWR** for fetching data from the API (don't use raw fetch for GET requests)
- SWR provides caching, revalidation, error handling, and loading states
- Create custom hooks in `/lib/hooks/` for data fetching
- Pattern:
```tsx
...
export function useUserOrganizations() {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    user ? [`${process.env.NEXT_PUBLIC_API_URL}/v1/organizations/user`, user.getIdToken?.()] : null,
    ([url, tokenPromise]) => tokenPromise.then((token: string) => fetcher(url, token)),
    {
...
export function useOrganization(orgId: string) {
  ...
  const { data, error, isLoading } = useSWR(
    user && orgId ? [`${process.env.NEXT_PUBLIC_API_URL}/v1/organizations/${orgId}`, user.getIdToken?.()] : null,
    ([url, tokenPromise]) => tokenPromise.then((token: string) => fetcher(url, token)),
    {
      revalidateOnFocus: false,
    }
  ...
  return (
    <div>
      {organizations?.map(org => (
        <Card key={org.id}>{org.org_name}</Card>
      ))}
    </div>
  )
}
...
- Use raw fetch for mutations (POST, PUT, DELETE)
- Call `mutate()` after mutation to revalidate cache
- Pattern:
```tsx
const onSubmit = async (data: FormData) => {
  try {
    const token = await user?.getIdToken?.()
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/v1/organizations`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
    ...
    mutate() // Revalidate organizations list
    return result
  } catch (error) {
    // Handle error
  }
...
#### SWR Configuration
- `revalidateOnFocus: false` - Don't revalidate when window regains focus (good for long-running sessions)
- `revalidateOnReconnect: true` - Revalidate when connection is restored
- `dedupingInterval: 2000` - Default, avoid duplicate requests within 2 seconds
- `focusThrottleInterval: 5 * 60 * 1000` - Only revalidate on focus if 5+ minutes passed

#### Error Handling
```tsx
const { organizations, error } = useUserOrganizations()

if (error?.message === 'API error: 403') {
  return <div>You don't have access to this organization</div>
}

if (error) {
  return <div>Failed to load organizations: {error.message}</div>
}
```
